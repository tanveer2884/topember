...
class BillingAndCheckout extends Component
{
    ...
    public function selectShipping()
    {
        ...
        $this->shipping_first_name = optional($address)->first_name;
        $this->shipping_last_name = optional($address)->last_name;
        $this->shipping_email = optional($address)->email;
        $this->shipping_phone = optional($address)->phone;
        $this->shipping_address = optional($address)->address;
        $this->shipping_state = optional($address)->state;
        $this->shipping_city = optional($address)->city;
        $this->shipping_zip_code = optional($address)->zipCode;
    }

    public function selectBilling()
    {
        $address = Address::find($this->billing_address_id);
        $this->billing_first_name = optional($address)->first_name;
        $this->billing_last_name = optional($address)->last_name;
        $this->billing_email = optional($address)->email;
        $this->billing_phone = optional($address)->phone;
        $this->billing_address = optional($address)->address;
        $this->billing_state = optional($address)->state;
        $this->billing_city = optional($address)->city;
        $this->billing_zip_code = optional($address)->zipCode;
    }
    ...
    public function initializeFields()
    {
        ...
        $this->shipping_email = optional($shippingAddress)['email'];
        $this->shipping_phone = optional($shippingAddress)['phone'];
        ...
        $this->billing_email = optional($billingAddress)['email'];
        $this->billing_phone = optional($billingAddress)['phone'];
    ...
    public function getBillingShippingRules($type)
    {
        return [
            ...
            $type . '_email' => 'required|email|max:191',
            $type . '_phone' => 'required|max:20',
