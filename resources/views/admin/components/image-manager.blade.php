<div class="shadow sm:rounded-md">
    <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
        ...
        <div>
            <x-admin.components.input.fileupload label="<span class='plus text-gray-400'>+</span>"
                wire:model="{{ $wireModel }}" :images="$images" :filetypes="$filetypes" :maxFiles="$maxFiles" :maxFileSize="$maxFileSize"
                :multiple="$multiple" />
        </div>
            ...
            <div wire:sort sort.options='{group: "images", method: "sort"}' class="relative mt-4 space-y-2">
                @foreach ($this->images as $image)
                    <div class="flex items-center justify-between p-4 bg-white border rounded-md shadow-sm"
                        sort.item="images" sort.id="{{ $image['sort_key'] }}" wire:key="image_{{ $image['sort_key'] }}">
                        <div class="flex items-center w-full space-x-6">
                            ...
                            <div class="w-full">
                                <x-admin.components.input.text wire:model="images.{{ $loop->index }}.caption"
                                    placeholder="Enter Alt. text" />
                            </div>

                            <div class="flex items-center ml-4 space-x-4">
                                <x-admin.components.tooltip text="Make primary">
                                    <x-admin.components.input.toggle :disabled="$image['primary']" :on="$image['primary']"
                                ...
                                <button type="button" wire:loading.attr="disabled" wire:target="removeImage"
