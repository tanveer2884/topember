            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            <form action="{{ route(config('category.routeNamePrefix').'categories.update',$category) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-md-6">
                                        <select class="form-control" name="parent" >
                                            ...
                                            @foreach($categories as $cat)
                                                <option value="{{$cat->id}}" {{ old('parent',optional($category->parentCategory)->id) == $cat->id ? 'selected' :'' }}>{{ $cat->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
                                        <div class="help-block text-danger"> {{ $message }} </div>
                                ...
                                <div class="controls row mb-1 align-items-center">
                                    ...
                                    <div class="col-md-6">
                                        <select class="form-control" name="status" >
                                            ...
                                            <option value="0" {{ old('status',$category->is_active) == 0 ? 'selected' : '' }}>Disabled</option>
                                        </select>
                                        @error('status')
                                        <div class="help-block text-danger"> {{ $message }} </div>
                                        @enderror
